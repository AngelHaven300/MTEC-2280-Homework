...
let portName = 'COM5'; // fill in your serial port name
...
function draw() 
{
  ...
  if (!firstContact)  //if we have not yet received any data...
  {
    ...
    strokeWeight(3);
    stroke('#73115e');
    text("Your bestfriend is in need of help.", width/2, height/2);
  ...
  if(rxFlag)  //if rxFlag is true, we want to receive data, so...
  {
    ...
    if (inString.length > 0) 
    {
      ...
      {// print the incoming string to the console
        sensors = split(arduinoCom, ','); // split the string into an array of sensor values
        ...
        if(sensors.length >= 3) // check if we have all 4 sensor values before trying to access them
        {
          ...
          let rawCash = Number(sensors[2]);
          cashPot = map(rawCash, 0, 1023, 5, 200);
          cashPot = floor(cashPot); // round the potentiometer value to an integer
          if (cashPot > totalCash)
          {
            cashPot = totalCash;
          }
