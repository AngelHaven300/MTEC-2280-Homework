...
function draw() //  draw function loops forever at frame rate
{
  ...
  background(200, inData, 200);  // clear frame with background color each draw loop
  ...
  let sW = map(inData, 0, 255, 2, 10);
  strokeWeight(sW);
  circle(width/2, height/2, diameter);  // draw circle in center of canvas with diameter based on serial data

  rectMode(CENTER);
  fill(inData, 200, 255);
  let recW = map(inData, 0, 255, 5, width - 10);
  let recH = map(inData, 0, 255, 5, 100);
  rect(width/2, height/2, recW, recH, 20);
  ...
  if (inData >= 255) {
    ...
    stroke(random(50));
    text('yo momma', width/2, height - height/3);
  }
}
